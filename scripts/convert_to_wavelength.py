...
sys.path.append('/home/alaroche/xp_vae')
from xp_vae.model import ScatterVAE

model = ScatterVAE().to(device)
weights = torch.load('../models/APOGEE_MODEL/weight.pt')
...
import sys
sys.path.append('../..')
from astroNN_stars_foundation.utils import gaia_utils
...
def xp_to_lam(i):
    ...
    return gaia_utils.xp_coeffs_phys(dict_)[wavelength_mask_xp]
...
np.save('../vae_wavelength_space.npy',vae_wavelength_space)
